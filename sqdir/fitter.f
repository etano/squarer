      subroutine fitter( uold,diag,rv,lpos,fitdm,chis,norder,
...
c if bothsz=true we do a 2d fit in s and z. else a 1d fit in s only.
     ...
     .,totalchi,totalwt,eta(nexp),lhs(nexp,nexp),rhs(nexp)
     .,z(nexp),rcond,bf(nx)
...
!           write (*,'(i5,3e12.5)')lul,ro,rozero,star(1)
!           write (*,'(6e12.5)')(f(id,1),id=0,lul)
!           write (*,'(6e12.5)')(ac(id,ic),id=0,lul)
...
!              if(rv(i).lt.8.d0) then
!              write (*,'(3e12.5,i5,2f7.2)') wt,ro,ua(1),i,r1,r2
!              iwr=iwr+1
!               if(iwr.eq.20)stop
!              stop
!              totalwt=totalwt+wc
!              totalchi=totalchi+wc*ua(1)**2
!              do if=1,nexp
!              ie=iexp*if+2
!              eta(if)=r1**(2-ie)+r2**(2-ie)
!    .       -r1*r2*c*(r1**(-ie)+r2**(-ie))
!              rhs(if)=rhs(if)+wc*ua(1)*eta(if)
!              enddo

!              do ie=1,nexp
!              do if=1,nexp
!              lhs(ie,if)=lhs(ie,if)+wc*eta(ie)*eta(if)
!              enddo
!              enddo
!              endif
...
!     write (*,*)' chisq-total ',sqrt(totalchi/totalwt)
!     do if=1,nexp
!     eta(i)=top(i)/bot(i)
!     eta(if)=rhs(if)/lhs(if,if)
!     write (*,*)iexp*if+2,eta(if),' chi ',sqrt((totalchi
!    .  -eta(if)*rhs(if))/totalwt )
!     enddo

!     call dpoco(lhs,nexp,nexp,rcond,z,info)
!     call spofa(lhs,nexp,nexp,info)
!     if(info.ne.0) then
!        write (*,*) ' trouble in fitter info=',info
!        stop
!     endif
!     write (*,*) nexp,' condition number ',rcond
...
!     call dposl(lhs,nexp,nexp,eta)
!     call sposl(lhs,nexp,nexp,eta)

!     totalchi=totalchi/totalwt
!     do if=1,nexp
!     write (*,*) iexp*if+2,eta(if),totalchi
!     totalchi=totalchi-eta(if)*rhs(if)/totalwt
!     enddo
!     write (*,*)sqrt(totalchi)
!     do i=1,nx
!     bf(i)=0.d0
!      do if=1,nexp
!       bf(i)=bf(i)+eta(if)*rv(i)**(-iexp*if-2)
!      enddo
!     enddo
