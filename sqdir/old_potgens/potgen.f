...
c store away the potential specification
        ...
        enddo
        np=n-1
        if(p(2).eq.'COULOPT')then
           open (4,file=fname(1:ln)//'.vk',status='unknown')
           open (2,file=fname(1:ln)//'.vkall',status='unknown')
        endif
...
c     write (9,969) (diff(i),i=1,nx)
      ...
      parameter (mnkv=3000,mnsh=500,mdim=3,mnshex=6000)
...
c write out k-space potential. Unit 4 should have been opened already.
          ...
          close(4)
! new coding to write out all the vlr table !2.20.07
          do k=1,nshex
           write (2,'(2f10.5,2e14.6)') rknorm(k),wtk(k),vlr(k),vkbare(k)
          enddo
       endif
