...
1010  format (5e16.7)
      ...
      if (p(2).ne.'SCCOULEW')  then
       ...
       read(*,*) idec
       if (idec.eq.1) then
!       call cubspline(nx,rv,pott(1,1),cutr,20)
c       call cubspline(nx,rv,pott(1,2),cutr)
        write (11,'(2g20.10)') (rv(i),pott(i,1),i=1,nx)
        close(11)
       endif
      endif
...
c     enddo
     ...
     &                alp,pott2)
      ...
      parameter (mnkv=50000,mnsh=20000,mdim=3,npts=34,mnshex=6000)
      integer kmult(0:mnsh),ln,indx
      real*8 tpiell(mdim),rkcomp(mdim,mnkv),rknorm(mnsh),work(550)
     +  ,vkbare(mnsh),wtk(mnsh),ell(0:mdim),cent,epsil(mnsh)
     & ,aa,r0,pb,ab(9,4),rr(10)
     & ,rgrid(npts),vgrid(npts)
     & ,dr,pa2,pb1,bb(6),b(0:maxknots*(maxm+1)-1)
     & ,vlr(mnshex),rknot(0:maxknots),sm(0:maxm,0:2*maxm+1)
      parameter (dr=0.1d0)
     ...
     +  (c6+a2i*(c8+a2i*c10)))
      ...
      elseif(p(1).eq.'MYUKA') then
       if(icall.eq.1) then
        ...
        write(*,*) 'cutr, eps=',cutr,eps
        write(*,*) 'tfk,pa2,pb1=',tfk,pa2,pb1
       endif
       pott=eps*dexp(-tfk*rv)/rv
       if(n.ge.6) then
        pott=pb1*pott+(1.d0-pb1)*eps*dexp(-pa2*rv)/rv
       endif
...
c generate the set of kvectors
     ...
     +      ,nvects,mnkv,mnsh,mdim)
      ...
      call fillk_old(rknorm,wtk,kmult,nshlls,nshex1,argek,ndim,vol,mnsh)
         sangle=sang(ndim)
...
c
         if (lunit.ne.'A') then
          if (lunit.eq.'A0') then
           ...
           write (*,*) 'unit of lenght not supported: lunit=',lunit
           stop
          endif
         endif
...
c     write (44,*) '#  k     Vlr(k)  '
      ...
      subroutine fillk_old(rknorm,wtk,kmult,nshlls,nshex1,argek,ndim,vol
     +         ,mnsh)
      implicit none
