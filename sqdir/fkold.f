...
c  Jan 3, 1996
      ...
      parameter (mx=127,mdim=3,mdiv=8,mnts=400,mlevels=5
     +, mterms=10,mnprm=10,mnp=4,mnsys1=1+2**mdiv)
...
99    format(a14)
      ...
      ntypes=0
      ndim=0
1     j=ipickoff(21,p,n,10)
      if(j.eq.0) stop
c bug if file is empty
...
c       write (*,*)' spatial dimension=',ndim

      elseif(p(1).eq.'GRID') then
        nx=intread(p(2))
        call setgrid(mnts,nx,rv,p(3))
c       write (*,*) nx,rv(1),rv(nx)
...
c       write (6,*)' finished reading potential'
...
c beta is the 1/temperature
      ...
      write (*,*)' no terms in action ',ntermact
        call readseln(21,dmtable,mnts,4,mterms,mlevels
     +,ntermact,nlvld,beta,nx,1,nx,0)
        call readseln(21,dmtable(1,1,1,1,2),mnts,4,mterms,mlevels
     +,ntermact,nlvld,beta,nx,2,nx,0)
        call readseln(21,dmtable(1,1,1,1,3),mnts,4,mterms,mlevels
     +,ntermact,nlvld,beta,nx,3,nx,0)
...
c ndiv = number of bisections (work goes as nsteps*2**ndiv)
      ...
      do ip=1,nparts
         read (1,*) ((r(1,l,ip,j),l=1,ndim),j=1,2)
...
c compute tablulated results
      call toffd(r,r(1,1,1,2),mx,mdim,1,ndim,nparts,utab)
      eprim=nparts*ndim/(2*beta)-s2/(4*hbs2m*beta**2)
      utab(1,2)=utab(1,2)+eprim+vbar
      utab(1,3)=utab(1,3)+eprim
      betai=1.d0/beta
...
c     do k=1,np
c     if(sprng().lt.exp(vcut-wt(k))) then
c        ifout=1
...
c extrapolate
     ...
     +     /(nblock-1.d0))
      ...
      write (*,1020) (idiv,(u(idiv,l),uerr(idiv,l),l=1,3),idiv=1,ndiv)
      write (*,1022) (uextra(l),exrerr(l),l=1,3)
      write (*,1024) (utab(1,l),l=1,3)
      write (*,1021) 'MC-tab',((uextra(l)-utab(1,l)),l=1,3)
      write (*,1021) 'chi   ',((uextra(l)-utab(1,l))/exrerr(l),l=1,3)
      write (*,1021) 'bias  ',((u(ndiv,l)-av(ndiv,l))/nblock,l=1,3)
      write (*,1021) 'rel.ex',((uextra(l)-u(ndiv,l))/exrerr(l),l=1,3)
...
1040  format(/' time ',f20.3,'CPU seconds')
      ...
      implicit real*8 (a-h,o-z)
      parameter (mxx=127,mnts=400,mlevels=5,mterms=10)
       common/ch/hbs2m,pot(mnts,4)
     +,dmtable(mnts,4,mterms,mlevels,3),ntermact
...
4     ebeta(k)=ebeta(k)+pot(ind(k),1)+p(k)*(pot(ind(k),2)+p(k)*
     +  (pot(ind(k),3)+p(k)*pot(ind(k),4))   )
      ...
      subroutine toffd(r1,r2,mx,mdim,nx,ndim,nparts,u)
      implicit real*8 (a-h,o-z)
      parameter (mxx=127,mnts=400,mlevels=5,mterms=10)
       common/ch/hbs2m,pot(mnts,4)
     +,dmtable(mnts,4,mterms,mlevels,3),ntermact
...
c the end point term for r1
      ...
      call evpol(dmtable,ind,p,nx,mnts,uend,rdpdr,dvdr)
      call evpol(dmtable(1,1,1,1,2),ind,p,nx,mnts,uend(1,2),rdpdr,dvdr)
      call evpol(dmtable(1,1,1,1,3),ind,p,nx,mnts,uend(1,3),rdpdr,dvdr)
...
c now off-diagonal terms
      if(ntermact.gt.1) then
       ...
       call vsum2(nx,ind,p,dmtable(1,1,2,1,1),mnts,var,ntermact-1
     +  ,mxx,4,ndim,u)
       call vsum2(nx,ind,p,dmtable(1,1,2,1,2),mnts,var,ntermact-1
     +  ,mxx,4,ndim,u(1,2))
       call vsum2(nx,ind,p,dmtable(1,1,2,1,3),mnts,var,ntermact-1
     +  ,mxx,4,ndim,u(1,3))
      endif
