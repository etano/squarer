...
c                                    check for possible overflow.
      ...
      do 25 nn=nstart,nend
         pold = plast
         plast = p
         p = (float(2*nn))*plast/tempa-rsign*pold
         if(p-tover) 25, 25, 35
   25 continue
      nn = nend
...
c                                    for nbmx.gt.2.
c
      test = max(test,sqrt(plast*1.e14)*sqrt(2.*p))
c
...
c                                    passed.
      m = 1
      tempb = p/plast
      tempc = (float(nn+1))/tempa
      if(tempb+1./tempb.gt.2.*tempc)tempb=tempc+sqrt(tempc**2-1.)
      test = test/sqrt(tempb-1./tempb)
      if(p-test) 30, 55, 55
...
c                                    test is passed.
      test = .5*pold*plast*(1.-1./tempb**2)*1.e-14
      p = plast*tover
      nn = nn-1
      nend = min0(n,nn)
      do 45 ncalc=nstart,nend
         pold = psavel
         psavel = psave
         psave = (float(2*nn))*psavel/tempa-rsign*pold
...
c                                    store b(nn), until nn=2
      do 95 l=1,nend
         nn = nn-1
         b(nn) = ((float(2*nn))*b(nn+1))/arg-rsign*b(nn+2)
         m = 2-m
         sum = sum+(float(m))*b(nn)
   95 continue
c                                  calculate b(1)
