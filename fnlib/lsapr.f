      subroutine lsapr(n,c,z,ize,iperm,dm,dp,ys,yt,ivor,la,mnp)
...
c minimize z=sum_i c(iperm(i),i)
      implicit none
      integer n,mnp,i,j,j0,iu,ius,index,iw,iws,ind
     & ,ize(mnp),iperm(n),ivor(mnp)
      real*8 z,eps,sup,cc, ui,vj,d,vgl
     & , c(mnp,n),ys(mnp),yt(mnp),dm(mnp),dp(mnp)
      logical la(mnp)
